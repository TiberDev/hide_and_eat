...
public class PlayerRegistry : NetworkBehaviour, INetworkRunnerCallbacks
{
    ...
    [Networked, Capacity(CAPACITY)] NetworkDictionary<PlayerRef, PlayerObject> ObjectByRef { get; }
    ...
    bool GetAvailable(out byte index)
    {
        ...
        if (ObjectByRef.Count == CAPACITY)
        {
            index = 0;
            return false;
        }

        byte[] indices = ObjectByRef.OrderBy(kvp => kvp.Value.Index).Select(kvp => (byte)kvp.Value.Index).ToArray();
    ...
    public void Server_Add(NetworkRunner runner, PlayerRef pRef, PlayerObject pObj)
    {
    ...
    public void Server_Remove(NetworkRunner runner, PlayerRef pRef)
    {
        Debug.Assert(runner.IsServer);
    ...
    {
        Debug.Log("Host Shutdown: " + shutdownReason);
        Menu.Instance.SetScreen(Menu.ScreenType.JoinRoom);
        return;
        if(shutdownReason == ShutdownReason.Ok && Runner.IsServer)
        {
            Debug.Log("Host has closed the game.");
            Menu.Instance.SetScreen(Menu.ScreenType.JoinRoom);
        }
        else if (shutdownReason == ShutdownReason.DisconnectedByPluginLogic && runner.IsClient)
        {
            Debug.Log("Host has closed the game for clients.");
            Menu.Instance.SetScreen(Menu.ScreenType.JoinRoom);            
        }
    }
